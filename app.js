...
function handleSubmit(event) {
  ...
  if (formType === 'visitor') {
    ...
    const church = { id: `church-${Date.now()}`, name, city: String(data.get('city') || 'Brasil'), initials: initials(name), logoSymbol: initials(name).slice(0, 2), logoImage: '', appearance: { ...DEFAULT_APPEARANCE }, members: 0, status: 'Ativa', plan: 'Essencial' };
    ...
    church.city = String(data.get('churchCity') || church.city).trim();
    ...
    pendingLogoImage = null;
    saveState(); render(); showToast('Nome e identidade visual da igreja atualizados.');
  }
}
